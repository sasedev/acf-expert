<?php
namespace Acf\ResBundle\Controller;
    ...
    public function tempfileAction($file = null)
    {
        ...
        if ($this->endswith($file, '/')) {
            if ($fsAvatar->has($file)) {
                $this->gvars['title'] = $this->translate('indexof', array(
                    '%dir%' => $this->generateUrl('vfs_tmp_files', array(
                        'file' => $file
                    ))
                ));

                $this->gvars['pagetitle'] = $this->translate('indexof_raw', array(
                    '%dir%' => $this->generateUrl('vfs_tmp_files', array(
                        'file' => $file
                    ))
                ));
                ...
                return $this->renderResponse('AcfResBundle:Vfs:list_temp_files.html.twig', $this->gvars);
            } else {
