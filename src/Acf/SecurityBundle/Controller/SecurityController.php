    ...
    public function registerPostAction()
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($newUserForm->isValid()) {
                ...
                $role = $em->getRepository('AcfDataBundle:Role')->findOneBy(array(
                    'name' => 'ROLE_USER'
                ));
                ...
                $this->flashMsgSession('success', $this->translate('_security.register.success', array(
                    '%mail%' => $user->getEmail()
                )));
    ...
    public function lostPasswordAction()
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($lostPasswordForm->isValid()) {
                ...
                $em = $this->getEntityManager();
                $user = null;
                $user = $em->getRepository('AcfDataBundle:User')->findOneBy(array(
                    'username' => $username
                ));
                ...
                if (null != $user) {
                    ...
                    if (null == $user->getRecoveryExpiration() || $user->getRecoveryExpiration() < $now) {
                        ...
                        $user->setRecoveryCode(User::generateRandomChar(20, 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'));
                        $em->persist($user);
                        $em->flush();
                        ...
                        $this->flashMsgSession('success', $this->translate('_security.lostPassword.mail.newpassSent', array(
                            '%mail%' => $user->getEmail()
                        )));
                    ...
                    $this->flashMsgSession('error', $this->translate('_security.lostPassword.notfound', array(
                        '%username%' => $username
                    )));
                }
            }
