...
class AgendaController extends BaseController
{
	...
	public function indexAction()
	{
		...
		$chooseDateForm = $this->createForm(ChooseDateTForm::class, null, array('date' => new \DateTime('now')));
	...
	public function planningAction($year, $week)
	{
		if (null == $year || $year < 1) {
			...
			return $this->redirect($this->generateUrl('_security_agenda_planning', array('year' => $currentYear, 'week' => $currentWeek)));
		} elseif (null == $week) {
			$currentYear = $year;
			$currentWeek = date("W");
		...
		$chooseDateForm = $this->createForm(ChooseDateTForm::class, null, array('date' => new \DateTime($chooseDate->format('Y-M-d'))));
		$this->gvars['ChooseDateForm'] = $chooseDateForm->createView();
	...
	public function ajaxGotoDateAction()
	{
		...
		if (isset($reqData['ChooseDateForm'])) { //  && $request->isXmlHttpRequest()
			...
			if ($eventAddForm->isValid()) {
				...
				try {
					...
					$jsonResponse->setData(
						array('id' => $event->getId(), 'title' => $event->getTitle(), 'backgroundColor' => $event->getBackgroundColor(),
							'borderColor' => $event->getBorderColor(), 'textColor' => $event->getTextColor(),
							'dtStart' => $event->getEvStart(), 'dtEnd' => $event->getEvEnd(),
							'msg' => $this->translate('Agenda.add.success.txt', array('%agenda%' => $event->getTitle()))));
	...
	public function ajaxEditAction()
	{
		...
		if ($request->isXmlHttpRequest()) {
			...
			$response = new Response();
			try {
				$event = $em->getRepository('AcfDataBundle:Agenda')->findOneBy(array('id' => $id));
				...
				$jsonResponse->setData(
					array('id' => $event->getId(), 'title' => $event->getTitle(), 'backgroundColor' => $event->getBackgroundColor(),
						'borderColor' => $event->getBorderColor(), 'textColor' => $event->getTextColor(), 'dtStart' => $event->getEvStart(),
						'dtEnd' => $event->getEvEnd(),
						'msg' => $this->translate('Agenda.edit.success.txt', array('%agenda%' => $event->getTitle()))));
			...
			try {
				...
				$jsonResponse->setData(
					array('id' => $event->getId(), 'title' => $event->getTitle(), 'backgroundColor' => $event->getBackgroundColor(),
						'borderColor' => $event->getBorderColor(), 'textColor' => $event->getTextColor(), 'dtStart' => $event->getEvStart(),
						'dtEnd' => $event->getEvEnd(),
						'msg' => $this->translate('Agenda.delete.success.txt', array('%agenda%' => $event->getTitle()))));
	...
	public function eventAction($uid)
	{
		...
		try {
			...
			if (null == $event) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.agenda.edit', array('%agenda%' => $event->getTitle()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.agenda.edit.txt', array('%agenda%' => $event->getTitle()));
				...
				if (isset($reqData['EventEditForm'])) {
					...
					if ($eventEditForm->isValid()) {
						...
						$this->flashMsgSession('success',
							$this->translate('Agenda.edit.success.txt', array('%agenda%' => $event->getTitle())));
					...
					if ($eventEditUsersForm->isValid()) {
						...
						foreach ($event->getUsers() as $sharedUser) {
							...
							$message = \Swift_Message::newInstance()->setFrom($from, $fromName)
								->setTo($sharedUser->getEmail(), $sharedUser->getFullname())
								->setSubject($subject)
								->setBody($this->renderView('AcfSecurityBundle:Agenda:mail.html.twig', $mvars), 'text/html');
