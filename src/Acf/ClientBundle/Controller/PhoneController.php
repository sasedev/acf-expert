...
class PhoneController extends BaseController
{
		...
		try {
			...
			if (null == $phone) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Phone.delete.success', array('%phone%' => $phone->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $phone) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.phone.edit', array('%phone%' => $phone->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.phone.edit.txt', array('%phone%' => $phone->getLabel()));
			...
			if (null == $phone) {
				...
				if (isset($reqData['PhoneUpdateForm'])) {
					...
					if ($phoneUpdateForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Phone.edit.success', array('%phone%' => $phone->getLabel()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Phone.edit.failure', array('%phone%' => $phone->getLabel())));
					}
				}
	...
	protected function traceEntity(Phone $clonePhone, Phone $phone) {
		...
		$trace->setCompanyId($phone->getCompany()->getId());
		...
		$trace->setActionId2($phone->getCompany()->getId());
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Phone.traceEdit',
					array('%phone%' => $phone->getPhone(), '%company%' => $phone->getCompany()->getCorporateName())
					).$msg
				);
