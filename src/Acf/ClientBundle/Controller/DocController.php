...
use Acf\ClientBundle\Form\Doc\UpdateContentTForm as DocUpdateContentTForm;
use Acf\ClientBundle\Form\Doc\UpdateDescriptionTForm as DocUpdateDescriptionTForm;
use Acf\ClientBundle\Form\Doc\UpdateOriginalNameTForm as DocUpdateOriginalNameTForm;
...
class DocController extends BaseController
{
    ...
    public function deleteAction($uid)
    {
        ...
        try {
            ...
            if (null == $doc) {
                ...
                if (null == $companyUser || $companyUser->getDeleteDocs() == CompanyUser::CANT) {
                    $this->flashMsgSession('error', $this->translate('CompanyUser.accessForbidden'));
                ...
                $this->flashMsgSession('success', $this->translate('Doc.delete.success', array(
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $doc) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.doc.edit', array(
                    '%doc%' => $doc->getOriginalName()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.doc.edit.txt', array(
                    '%doc%' => $doc->getOriginalName()
                ));

                return $this->renderResponse('AcfClientBundle:Doc:edit.html.twig', $this->gvars);
            }
        } catch (\Exception $e) {
            $logger = $this->getLogger();
            $logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
    ...
    public function editPostAction($uid)
    {
        ...
        try {
            ...
            if (null == $doc) {
                ...
                if (null == $companyUser || $companyUser->getEditDocs() == CompanyUser::CANT) {
                    $this->flashMsgSession('error', $this->translate('CompanyUser.accessForbidden'));
                ...
                if (isset($reqData['DocUpdateDescriptionForm'])) {
                    ...
                    if ($docUpdateDescriptionForm->isValid()) {
                        $em->persist($doc);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('Doc.edit.success', array(
                            '%doc%' => $doc->getOriginalName()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('Doc.edit.failure', array(
                            '%doc%' => $doc->getOriginalName()
                        )));
                    }
    ...
    protected function traceEntity(Doc $cloneDoc, Doc $doc)
    {
        ...
        if ($msg != '') {
            ...
            $trace->setMsg($this->translate('Doc.traceEdit', array(
                '%doc%' => $doc->getOriginalName(),
                '%company%' => $doc->getCompany()
                    ->getCorporateName()
            )) . $msg);
