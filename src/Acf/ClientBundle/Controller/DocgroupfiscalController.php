...
class DocgroupfiscalController extends BaseController
{
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupfiscal) {
				...
				$docgroupfiscalUpdateDocsForm = $this->createForm(DocgroupfiscalUpdateDocsTForm::class, $docgroupfiscal,
					array('company' => $docgroupfiscal->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupfiscal.edit',
					array('%docgroupfiscal%' => $docgroupfiscal->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupfiscal.edit.txt',
					array('%docgroupfiscal%' => $docgroupfiscal->getLabel()));
				...
				if (isset($reqData['DocNewForm'])) {
					...
					if ($docgroupfiscalUpdateDocsForm->isValid()) {
						...
						$this->flashMsgSession('error',
							$this->translate('Docgroupfiscal.edit.failure', array('%docgroupfiscal%' => $docgroupfiscal->getLabel())));
					}
				}
	...
	protected function traceEntity(Docgroupfiscal $cloneDocgroupfiscal, Docgroupfiscal $docgroupfiscal)
	{
		...
		if ($cloneDocgroupfiscal->getParent() != $docgroupfiscal->getParent() &&
			(($cloneDocgroupfiscal->getParent() == null && $docgroupfiscal->getParent() != null) ||
			($cloneDocgroupfiscal->getParent() != null && $docgroupfiscal->getParent() == null) ||
			($docgroupfiscal->getParent() != null && $cloneDocgroupfiscal->getParent() != null &&
			$cloneDocgroupfiscal->getParent()->getPageUrlFull() != $docgroupfiscal->getParent()->getPageUrlFull()))) {
		...
		if (\count(\array_diff($docgroupfiscal->getDocs()->toArray(), $cloneDocgroupfiscal->getDocs()->toArray())) != 0 ||
			\count(\array_diff($cloneDocgroupfiscal->getDocs()->toArray(), $docgroupfiscal->getDocs()->toArray())) != 0) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate('Docgroupfiscal.traceEdit',
					array('%docgroupfiscal%' => $docgroupfiscal->getLabel(),
						'%company%' => $docgroupfiscal->getCompany()
