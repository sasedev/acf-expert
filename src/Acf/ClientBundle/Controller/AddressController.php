...
use Acf\ClientBundle\Form\Address\UpdateTForm as AddressUpdateTForm;
...
class AddressController extends BaseController
{
    ...
    public function deleteAction($uid)
    {
        ...
        try {
            ...
            if (null == $address) {
                ...
                if (null == $companyUser || $companyUser->getDeleteAddresse() == CompanyUser::CANT) {
                ...
                $this->flashMsgSession('success', $this->translate('Address.delete.success', array(
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $address) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.address.edit', array(
                    '%address%' => $address->getLabel()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.address.edit.txt', array(
                    '%address%' => $address->getLabel()
                ));

                return $this->renderResponse('AcfClientBundle:Address:edit.html.twig', $this->gvars);
            }
        } catch (\Exception $e) {
            $logger = $this->getLogger();
            $logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
    ...
    public function editPostAction($uid)
    {
        ...
        try {
            ...
            if (null == $address) {
                ...
                if (null == $companyUser || $companyUser->getEditAddresse() == CompanyUser::CANT) {
                    $this->flashMsgSession('error', $this->translate('CompanyUser.accessForbidden'));
                ...
                if (isset($reqData['AddressUpdateForm'])) {
                    ...
                    if ($addressUpdateForm->isValid()) {
                        $em->persist($address);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('Address.edit.success', array(
                            '%address%' => $address->getLabel()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('Address.edit.failure', array(
                            '%address%' => $address->getLabel()
                        )));
                    }
                }
    ...
    protected function traceEntity(Address $cloneAddress, Address $address)
    {
        ...
        if ($msg != '') {
            ...
            $trace->setMsg($this->translate('Address.traceEdit', array(
