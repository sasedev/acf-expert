...
class DocgroupController extends BaseController
{
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroup) {
				...
				$docgroupUpdateDocsForm = $this->createForm(DocgroupUpdateDocsTForm::class, $docgroup,
					array('company' => $docgroup->getCompany()));

				$doc = new Doc();
				$doc->setCompany($docgroup->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $docgroup->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroup.edit', array('%docgroup%' => $docgroup->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroup.edit.txt',
					array('%docgroup%' => $docgroup->getLabel()));
				...
				if (isset($reqData['DocNewForm'])) {
					...
					if ($docgroupUpdateDocsForm->isValid()) {
						...
						$this->flashMsgSession('success',
							$this->translate('Docgroup.edit.success', array('%docgroup%' => $docgroup->getLabel())));
						$this->gvars['stabActive'] = 3;
						$this->getSession()->set('stabActive', 3);
						...
						$this->flashMsgSession('error',
							$this->translate('Docgroup.edit.failure', array('%docgroup%' => $docgroup->getLabel())));
					}
				}
	...
	protected function traceEntity(Docgroup $cloneDocgroup, Docgroup $docgroup)
	{
		...
		if ($cloneDocgroup->getParent() != $docgroup->getParent() &&
			(($cloneDocgroup->getParent() == null && $docgroup->getParent() != null) ||
			($cloneDocgroup->getParent() != null && $docgroup->getParent() == null) ||
			($docgroup->getParent() != null && $cloneDocgroup->getParent() != null &&
			$cloneDocgroup->getParent()->getPageUrlFull() != $docgroup->getParent()->getPageUrlFull()))) {
		...
		if (\count(\array_diff($docgroup->getDocs()->toArray(), $cloneDocgroup->getDocs()->toArray())) != 0 ||
			\count(\array_diff($cloneDocgroup->getDocs()->toArray(), $docgroup->getDocs()->toArray())) != 0) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate('Docgroup.traceEdit',
					array('%docgroup%' => $docgroup->getLabel(), '%company%' => $docgroup->getCompany()
