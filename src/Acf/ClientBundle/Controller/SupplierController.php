...
use Acf\ClientBundle\Form\Supplier\UpdateTForm as SupplierUpdateTForm;
use Acf\ClientBundle\Form\Supplier\UpdateDocsTForm as SupplierUpdateDocsTForm;
...
class SupplierController extends BaseController
{
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $supplier) {
				...
				if (null == $companyUser || $companyUser->getDeleteSuppliers() == CompanyUser::CANT) {
				...
				$this->flashMsgSession('success', $this->translate('Supplier.delete.success', array('%supplier%' => $supplier->getLabel())));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $supplier) {
				...
				$supplierUpdateDocsForm = $this->createForm(SupplierUpdateDocsTForm::class, $supplier,
					array('company' => $supplier->getCompany()));

				$doc = new Doc();
				$doc->setCompany($supplier->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $supplier->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.supplier.edit', array('%supplier%' => $supplier->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.supplier.edit.txt',
					array('%supplier%' => $supplier->getLabel()));

				return $this->renderResponse('AcfClientBundle:Supplier:edit.html.twig', $this->gvars);
			}
		} catch (\Exception $e) {
			$logger = $this->getLogger();
			$logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
	...
	public function editPostAction($uid)
	{
		...
		try {
			...
			if (null == $supplier) {
				...
				$companyUser = $em->getRepository('AcfDataBundle:CompanyUser')->findOneBy(array('company' => $company, 'user' => $user));
				if (null == $companyUser || $companyUser->getEditSuppliers() == CompanyUser::CANT) {
					$this->flashMsgSession('error', $this->translate('CompanyUser.accessForbidden'));
					return $this->redirect($this->generateUrl('_client_homepage'));
				}
				$this->gvars['companyUser'] = $companyUser;
				...
				if (isset($reqData['SupplierUpdateForm'])) {
					...
					if ($supplierUpdateForm->isValid()) {
						...
						$this->flashMsgSession('success',
							$this->translate('Supplier.edit.success', array('%supplier%' => $supplier->getLabel())));
						...
						$this->flashMsgSession('error',
							$this->translate('Supplier.edit.failure', array('%supplier%' => $supplier->getLabel())));
					}
	...
	protected function traceEntity(Supplier $cloneSupplier, Supplier $supplier)
	{
		...
		if (\count(\array_diff($supplier->getDocs()->toArray(), $cloneSupplier->getDocs()->toArray())) != 0 ||
			\count(\array_diff($cloneSupplier->getDocs()->toArray(), $supplier->getDocs()->toArray())) != 0) {
		...
		if (\count(\array_diff($supplier->getSectors()->toArray(), $cloneSupplier->getSectors()->toArray())) != 0 ||
			\count(\array_diff($cloneSupplier->getSectors()->toArray(), $supplier->getSectors()->toArray())) != 0) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate('Supplier.traceEdit',
					array('%supplier%' => $supplier->getLabel(), '%company%' => $supplier->getCompany()
