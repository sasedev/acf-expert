...
class DefaultController extends BaseController
{
    ...
    public function indexAction($month = null, $year = null)
    {
        ...
        foreach ($companies as $company) {
            ...
            $renderNaturesPrevYear = array();
            foreach ($companyNatures as $companyNature) {
                $renderNature = array();
                $val = $em->getRepository('AcfDataBundle:Buy')->achatHtByCompanyNatureInYear($companyNature, $prevYear);
                if (null != $val && $val != 0) {
                    $renderNature['nature'] = $companyNature;
                    $renderNature['val'] = $val;
                    $renderNaturesPrevYear[] = $renderNature;
                }
            }
            $renderCompany['nature_list_prev_year'] = $renderNaturesPrevYear;
