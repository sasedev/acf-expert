...
class DocgrouppersoController extends BaseController
{
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupperso) {
				...
				$docgrouppersoUpdateDocsForm = $this->createForm(DocgrouppersoUpdateDocsTForm::class, $docgroupperso,
					array('company' => $docgroupperso->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupperso.edit',
					array('%docgroupperso%' => $docgroupperso->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupperso.edit.txt',
					array('%docgroupperso%' => $docgroupperso->getLabel()));
				...
				if (isset($reqData['DocNewForm'])) {
					...
					if ($docgrouppersoUpdateDocsForm->isValid()) {
						...
						$this->flashMsgSession('success',
							$this->translate('Docgroupperso.edit.success', array('%docgroupperso%' => $docgroupperso->getLabel())));
						$this->gvars['stabActive'] = 3;
						$this->getSession()->set('stabActive', 3);
						...
						$this->flashMsgSession('error',
							$this->translate('Docgroupperso.edit.failure', array('%docgroupperso%' => $docgroupperso->getLabel())));
					}
				}
	...
	protected function traceEntity(Docgroupperso $cloneDocgroupperso, Docgroupperso $docgroupperso)
	{
		...
		if ($cloneDocgroupperso->getParent() != $docgroupperso->getParent() &&
			(($cloneDocgroupperso->getParent() == null && $docgroupperso->getParent() != null) ||
			($cloneDocgroupperso->getParent() != null && $docgroupperso->getParent() == null) ||
			($docgroupperso->getParent() != null && $cloneDocgroupperso->getParent() != null &&
			$cloneDocgroupperso->getParent()->getPageUrlFull() != $docgroupperso->getParent()->getPageUrlFull()))) {
		...
		if (\count(\array_diff($docgroupperso->getDocs()->toArray(), $cloneDocgroupperso->getDocs()->toArray())) != 0 ||
			\count(\array_diff($cloneDocgroupperso->getDocs()->toArray(), $docgroupperso->getDocs()->toArray())) != 0) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate('Docgroupperso.traceEdit',
					array('%docgroupperso%' => $docgroupperso->getLabel(), '%company%' => $docgroupperso->getCompany()
