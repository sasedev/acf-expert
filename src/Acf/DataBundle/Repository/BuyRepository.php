...
class BuyRepository extends EntityRepository
{
    ...
    {
        $qb = $this->createQueryBuilder('b')->select('sum(b.balanceTtc - b.vat - b.stamp)')->join('b.monthlyBalance', 'm')->join('m.company', 'c')->where('c.id = :id')->andWhere('m.year = :year')->andWhere('m.month = :month')->andWhere('(b.transactionStatus = :status1 OR b.transactionStatus = :status2)')->setParameter('id', $c->getId())->setParameter('year', \intval($year))->setParameter('month', \intval($month))->setParameter('status1', Transaction::STATUS_DONE)->setParameter('status2', Transaction::STATUS_PENDING);
        $query = $qb->getQuery();
    ...
    {
        $qb = $this->createQueryBuilder('b')->select('sum(b.balanceNet)')->join('b.monthlyBalance', 'm')->join('m.company', 'c')->where('c.id = :id')->andWhere('m.year = :year')->andWhere('m.month = :month')->andWhere('b.transactionStatus = :status')->andWhere('b.paymentType != :paymentType')->setParameter('id', $c->getId())->setParameter('year', \intval($year))->setParameter('month', \intval($month))->setParameter('status', Transaction::STATUS_DONE)->setParameter('paymentType', Transaction::PTYPE_NA);
        $query = $qb->getQuery();
    ...
    /**
     * Update CompanyNature Null By Company
     *
     * @param Company $c
     * @param CompanyNature $cn
     *
