...
class CompanyController extends BaseController
{
		...
		if (isset($reqData['CompanyNewForm'])) {
			...
			if ($companyNewForm->isValid()) {
				...
				$this->flashMsgSession('success',
					$this->translate('Company.add.success', array('%company%' => $company->getCorporateName())));
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $company) {
				...
				$this->flashMsgSession('success',
					$this->translate('Company.delete.success', array('%company%' => $company->getCorporateName())));
			}
		} catch (\Exception $e) {
			$logger = $this->getLogger();
			$logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $company) {
				...
				$customersConstStr = $em->getRepository('AcfDataBundle:ConstantStr')->findOneBy(
					array('name' => 'customersPrefix'));
				...
				$suppliersConstStr = $em->getRepository('AcfDataBundle:ConstantStr')->findOneBy(
					array('name' => 'suppliersPrefix'));
				...
				$banksConstStr = $em->getRepository('AcfDataBundle:ConstantStr')->findOneBy(
					array('name' => 'banksPrefix'));
				...
				$fundsConstStr = $em->getRepository('AcfDataBundle:ConstantStr')->findOneBy(
					array('name' => 'fundsPrefix'));
				...
				$withholdingsConstStr = $em->getRepository('AcfDataBundle:ConstantStr')->findOneBy(
					array('name' => 'withholdingsPrefix'));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.company.edit',
			...
			if (null == $company) {
				...
				if (isset($reqData['CompanyUpdateTypeForm'])) {
					...
					if ($phoneNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Phone.add.success', array('%phone%' => $phone->getLabel())));
					...
					if ($companyFrameNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('CompanyFrame.add.success',
								array('%companyFrame%' => $companyFrame->getFullName())));
					...
					if ($companyNatureNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('CompanyNature.add.success',
								array('%companyNature%' => $companyNature->getLabel())));
					...
					if ($companyNatureImportForm->isValid()) {
						...
						foreach ($iterator as $worksheet) {
							...
							$log .= "Feuille : '" . $worksheetTitle . "' trouvée contenant " . $highestRow .
								" lignes et " . $highestColumnIndex . " colonnes avec comme plus grand index " .
								$highestColumn . " <br>";
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							if ($label != "") {
								$companyNature = $em->getRepository('AcfDataBundle:CompanyNature')->findOneBy(
					...
					if ($docNewForm->isValid()) {
						...
						foreach ($docFiles as $docFile) {
							...
							$fileName = sha1(uniqid(mt_rand(), true)) . '.' .
								strtolower($docFile->getClientOriginalExtension());
					...
					if ($pilotNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Pilot.add.success', array('%pilot%' => $pilot->getMission())));
					...
					if ($companyUserNewForm->isValid()) {
						...
						if (!$has_client_role) {
							...
							$role_client = $em->getRepository('AcfDataBundle:Role')->findOneBy(array('name' => 'ROLE_CLIENT1'));
							if (null == $role_client) {
								$role_client = new Role();
								$role_client->setName('ROLE_CLIENT1');
							}

							$user->addUserRole($role_client);
							$em->persist($user);
						}
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('CompanyUser.add.success', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName())));
					...
					if ($companyAdminNewForm->isValid()) {
						...
						if (!$has_client_role) {
							...
							$role_admin = $em->getRepository('AcfDataBundle:Role')->findOneBy(array('name' => 'ROLE_ADMIN'));
							if (null == $role_admin) {
								$role_admin = new Role();
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('CompanyAdmin.add.success', array('%company%' => $companyAdmin->getCompany()->getCorporateName(), '%user%' => $companyAdmin->getUser()->getFullName())));
					...
					if ($companyLabelNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('CompanyLabel.add.success',
								array('%companyLabel%' => $companyLabel->getName())));
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							if ($abrev != "" && $name != "") {
								$companyLabel = $em->getRepository('AcfDataBundle:CompanyLabel')->findOneBy(
									array('company' => $company, 'abrev' => $abrev));
					...
					if ($bankNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Bank.add.success', array('%bank%' => $bank->getLabel())));
					...
					if ($bankImportForm->isValid()) {
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							if ($number != "" && $number > 0 && $label != "") {
								...
								$bank = $em->getRepository('AcfDataBundle:Bank')->findOneBy(
									array('company' => $company, 'number' => $number));
								$bank1 = $em->getRepository('AcfDataBundle:Bank')->findOneBy(
									array('company' => $company, 'label' => $label));
								if (null == $bank && null == $bank1) {
									...
									$lineUnprocessed ++;
									$log .= "le Banque à la ligne " . $lineRead . " existe déjà<br>";
								}
							} else {
								$lineError ++;
								$log .= "la ligne " . $lineRead . " contient des erreurs<br>";
					...
					if ($fundNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Fund.add.success', array('%fund%' => $fund->getLabel())));
					...
					if ($fundImportForm->isValid()) {
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							if ($number != "" && $number > 0 && $label != "") {
								...
								$fund = $em->getRepository('AcfDataBundle:Fund')->findOneBy(
									array('company' => $company, 'number' => $number));
								$fund1 = $em->getRepository('AcfDataBundle:Fund')->findOneBy(
									array('company' => $company, 'label' => $label));
								if (null == $fund && null == $fund1) {
									...
									$lineUnprocessed ++;
									$log .= "le Caisse à la ligne " . $lineRead . " existe déjà<br>";
								}
							} else {
								$lineError ++;
								$log .= "la ligne " . $lineRead . " contient des erreurs<br>";
						...
						$log .= $lineRead . " lignes lues<br>";
						$log .= $fundsNew . " nouveaux Caisses<br>";
					...
					if ($withholdingNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Withholding.add.success',
								array('%withholding%' => $withholding->getLabel())));
					...
					if ($withholdingImportForm->isValid()) {
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							if ($number != "" && $number > 0 && $label != "") {
								...
								if (null == $withholding && null == $withholding1) {
									...
									$lineUnprocessed ++;
									$log .= "le Retenue à la ligne " . $lineRead . " existe déjà<br>";
								}
							} else {
								$lineError ++;
								$log .= "la ligne " . $lineRead . " contient des erreurs<br>";
						...
						$log .= $lineRead . " lignes lues<br>";
						$log .= $withholdingsNew . " nouveaux Retenues<br>";
					...
					if ($mbsaleNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('MBSale.add.success', array('%mbsale%' => $mbsale->getRef())));
					...
					if ($docgroupcomptableNewForm->isValid()) {
						...
						$this->flashMsgSession('success',
							$this->translate('Docgroupcomptable.add.success',
					...
					if ($docgroupbankNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupbank.add.success',
					...
					if ($docgroupNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroup.add.success', array('%docgroup%' => $docgroup->getLabel())));
					...
					if ($shareholderNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Shareholder.add.success', array('%shareholder%' => $shareholder->getName())));
					...
					if ($docgroupfiscalNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupfiscal.add.success',
								array('%docgroupfiscal%' => $docgroupfiscal->getLabel())));
					...
					if ($docgrouppersoNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupperso.add.success',
								array('%docgroupperso%' => $docgroupperso->getLabel())));
					...
					if ($docgroupsystNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupsyst.add.success',
					...
					if ($docgroupauditNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupaudit.add.success',
								array('%docgroupaudit%' => $docgroupaudit->getLabel())));
	...
	public function shareholderExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.shareholder.listByCompany',
					array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function companyLabelExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.companyLabel.listByCompany',
					array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function companyNatureExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.companyNature.listByCompany',
					array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function pilotExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.pilot.listByCompany',
					array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function customerExcelAction($uid)
	{
		...
		try {
			...
			foreach ($customers as $customer) {
				...
				$workSheet->setCellValue('F' . $i, $customer->getCommercialRegister(),
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.customer.listByCompany', array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function supplierExcelAction($uid)
	{
		...
		try {
			...
			foreach ($suppliers as $supplier) {
				...
				$workSheet->setCellValue('E' . $i, $supplier->getCommercialRegister(),
					\PHPExcel_Cell_DataType::TYPE_STRING2);
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.supplier.listByCompany', array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function bankExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.bank.listByCompany', array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function fundExcelAction($uid)
	{
		...
		try {
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.fund.listByCompany', array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
	...
	public function withholdingExcelAction($uid)
	{
		...
		try {
			...
			foreach ($withholdings as $withholding) {
				...
				$workSheet->setCellValueExplicit('B' . $i, $numb, \PHPExcel_Cell_DataType::TYPE_STRING2);
				$workSheet->setCellValue('C' . $i, $withholding->getValue() . '%',
					\PHPExcel_Cell_DataType::TYPE_STRING2);
			...
			$filename = $this->normalize(
				$this->translate('pagetitle.withholding.listByCompany',
					array('%company%' => $company->getCorporateName())));
			$filename = str_ireplace('"', '|', $filename);
			$filename = str_ireplace(' ', '_', $filename);
