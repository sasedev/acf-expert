...
use Acf\AdminBundle\Form\MSalary\ImportTForm as MSalaryImportTForm;
use Acf\AdminBundle\Form\MPaye\UpdateDocsTForm as MPayeUpdateDocsTForm;
...
class MPayeController extends BaseController
{
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $mpaye) {
                ...
                $mpayeUpdateDocsForm = $this->createForm(MPayeUpdateDocsTForm::class, $mpaye, array(
                ...
                $this->gvars['MPayeUpdateDocsForm'] = $mpayeUpdateDocsForm->createView();
                ...
                return $this->renderResponse('AcfAdminBundle:MPaye:edit.html.twig', $this->gvars);
            }
        } catch (\Exception $e) {
    ...
    public function editPostAction($uid)
    {
        ...
        try {
            ...
            if (null == $mpaye) {
                ...
                if (isset($reqData['MSalaryImportForm'])) {
                    ...
                    if ($docNewForm->isValid()) {
                        ...
                        foreach ($company->getUsers() as $user) {
                            if ($user->hasRole($acfPayrollRole)) {
                                $users[] = $user;
                            }
                        }
                ...
                } elseif (isset($reqData['MPayeUpdateDocsForm'])) {
                    $this->gvars['tabActive'] = 3;
                    $this->getSession()->set('tabActive', 3);
                    $this->gvars['stabActive'] = 2;
                    $this->getSession()->set('stabActive', 2);
                    $mpayeUpdateDocsForm->handleRequest($request);
                    if ($mpayeUpdateDocsForm->isValid()) {
                        $em->persist($mpaye);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('MPaye.edit.success', array(
                            '%mpaye%' => $mpaye->getRef()
                        )));
                        $this->gvars['stabActive'] = 3;
                        $this->getSession()->set('stabActive', 3);
                        ...
                        $this->flashMsgSession('error', $this->translate('MPaye.edit.failure', array(
                            '%mpaye%' => $mpaye->getRef()
                        )));
                    }
                }
    ...
    public function excelAction($uid)
    {
        ...
        try {
            ...
            $filename = $this->normalizeString($this->normalize($this->translate('pagetitle.msalary.listByMPaye', array(
                '%mpaye%' => $mpaye->getRef(),
                '%company%' => $mpaye->getCompany()
                    ->getCorporateName()
            ))));
    ...
    public function excelYearAction($year, $uid)
    {
        ...
        try {
            ...
            if (null == $company) {
                ...
                $mpayes = $em->getRepository('AcfDataBundle:MPaye')->getAllByYearCompany($year, $company);
                $salaries = array();
                foreach ($mpayes as $mpaye) {
                    $salaries = array_merge($salaries, $mpaye->getSalaries()->toArray());
                }
                ...
                $filename = $this->normalizeString($this->normalize($this->translate('pagetitle.msalary.listByYear', array(
                    '%year%' => $year,
