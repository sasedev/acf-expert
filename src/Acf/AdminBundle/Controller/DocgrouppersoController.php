...
class DocgrouppersoController extends BaseController
{
		...
		try {
			...
			if (null == $docgroupperso) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Docgroupperso.delete.success', array('%docgroupperso%' => $docgroupperso->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupperso) {
				...
				$docgrouppersoUpdateParentForm = $this->createForm(DocgrouppersoUpdateParentTForm::class, $docgroupperso, array('selfUrl' => $docgroupperso->getPageUrlFull(), 'company' => $docgroupperso->getCompany()));
	...
	protected function traceEntity(Docgroupperso $cloneDocgroupperso, Docgroupperso $docgroupperso) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Docgroupperso.traceEdit',
					array('%docgroupperso%' => $docgroupperso->getLabel(), '%company%' => $docgroupperso->getCompany()->getCorporateName())
					).$msg
				);
