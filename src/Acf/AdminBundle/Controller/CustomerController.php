...
use Acf\AdminBundle\Form\Customer\UpdateTForm as CustomerUpdateTForm;
use Acf\AdminBundle\Form\Customer\UpdateDocsTForm as CustomerUpdateDocsTForm;
		...
		try {
			...
			if (null == $customer) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Customer.delete.success', array('%customer%' => $customer->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	protected function traceEntity(Customer $cloneCustomer, Customer $customer) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Customer.traceEdit',
					array('%customer%' => $customer->getLabel(), '%company%' => $customer->getCompany()->getCorporateName())
					).$msg
				);
