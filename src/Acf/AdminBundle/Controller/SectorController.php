...
class SectorController extends BaseController
{
        ...
        if (isset($reqData['SectorNewForm'])) {
            ...
            if ($sectorNewForm->isValid()) {
                ...
                if ($request->isXmlHttpRequest()) {
                    ...
                    return $this->renderResponse('AcfAdminBundle:Sector:add.ajax.html.twig', $this->gvars);
                } else {
                    $this->flashMsgSession('success', $this->translate('Sector.add.success', array(
                        '%sector%' => $sector->getLabel()
                    )));
                }

                return $this->redirect($this->generateUrl('_admin_sector_editGet', array(
                    'id' => $sector->getId()
                )));
            } else {
                ...
                if ($request->isXmlHttpRequest()) {
                    ...
                    return $this->renderResponse('AcfAdminBundle:Sector:add_error.ajax.html.twig', $this->gvars);
                } else {
                    $this->flashMsgSession('error', $this->translate('Sector.add.failure'));
                }
            }
        }
        $this->gvars['sector'] = $sector;
    ...
    public function deleteAction($id)
    {
        ...
        try {
            ...
            if (null == $sector) {
                ...
                $this->flashMsgSession('success', $this->translate('Sector.delete.success', array(
                    '%sector%' => $sector->getLabel()
                )));
            }
        } catch (\Exception $e) {
    ...
    public function editGetAction($id)
    {
        ...
        try {
            ...
            if (null == $sector) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.sector.edit', array(
                    '%sector%' => $sector->getLabel()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.sector.edit.txt', array(
                    '%sector%' => $sector->getLabel()
                ));
            ...
            if (null == $sector) {
                ...
                if (isset($reqData['SectorUpdateForm'])) {
                    ...
                    if ($sectorUpdateForm->isValid()) {
                        $em->persist($sector);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('Sector.edit.success', array(
                            '%sector%' => $sector->getLabel()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('Sector.edit.failure', array(
                            '%sector%' => $sector->getLabel()
                        )));
                    }
                }
    ...
    protected function traceEntity(Sector $cloneSector, Sector $sector)
    {
        ...
        if ($msg != '') {
            ...
            $trace->setMsg($this->translate('Sector.traceEdit', array(
                '%sector%' => $sector->getLabel()
            )) . $msg);
