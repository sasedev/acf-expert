		...
		try {
			...
			if (null == $pilot) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Pilot.delete.success', array('%pilot%' => $pilot->getMission()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $pilot) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.pilot.edit', array('%pilot%' => $pilot->getMission()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.pilot.edit.txt', array('%pilot%' => $pilot->getMission()));
			...
			if (null == $pilot) {
				...
				if (isset($reqData['PilotUpdateForm'])) {
					...
					if ($pilotUpdateForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Pilot.edit.success', array('%pilot%' => $pilot->getMission()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Pilot.edit.failure', array('%pilot%' => $pilot->getMission())));
					}
				}
	...
	protected function traceEntity(Pilot $clonePilot, Pilot $pilot) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Pilot.traceEdit',
					array('%pilot%' => $pilot->getRef(), '%company%' => $pilot->getCompany()->getCorporateName())
					).$msg
				);
