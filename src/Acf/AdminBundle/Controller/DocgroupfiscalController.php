...
class DocgroupfiscalController extends BaseController
{
		...
		try {
			...
			if (null == $docgroupfiscal) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Docgroupfiscal.delete.success', array('%docgroupfiscal%' => $docgroupfiscal->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupfiscal) {
				...
				$docgroupfiscalUpdateParentForm = $this->createForm(DocgroupfiscalUpdateParentTForm::class, $docgroupfiscal, array('selfUrl' => $docgroupfiscal->getPageUrlFull(), 'company' => $docgroupfiscal->getCompany()));
				$docgroupfiscalUpdateDocsForm = $this->createForm(DocgroupfiscalUpdateDocsTForm::class, $docgroupfiscal, array('company' => $docgroupfiscal->getCompany()));

				$doc = new Doc();
				$doc->setCompany($docgroupfiscal->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $docgroupfiscal->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupfiscal.edit', array('%docgroupfiscal%' => $docgroupfiscal->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupfiscal.edit.txt', array('%docgroupfiscal%' => $docgroupfiscal->getLabel()));
			...
			if (null == $docgroupfiscal) {
				...
				if (isset($reqData['DocgroupfiscalUpdateLabelForm'])) {
					...
					if ($docgroupfiscalUpdateLabelForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Docgroupfiscal.edit.success', array('%docgroupfiscal%' => $docgroupfiscal->getLabel()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Docgroupfiscal.edit.failure', array('%docgroupfiscal%' => $docgroupfiscal->getLabel())));
					}
	...
	protected function traceEntity(Docgroupfiscal $cloneDocgroupfiscal, Docgroupfiscal $docgroupfiscal) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Docgroupfiscal.traceEdit',
					array('%docgroupfiscal%' => $docgroupfiscal->getLabel(), '%company%' => $docgroupfiscal->getCompany()->getCorporateName())
					).$msg
				);
