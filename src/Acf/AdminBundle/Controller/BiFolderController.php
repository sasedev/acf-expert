		...
		if (isset($reqData['BiFolderNewForm'])) {
			...
			if ($biFolderNewForm->isValid()) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('BiFolder.add.success', array('%biFolder%' => $biFolder->getTitle()))
				);

				return $this->redirect(
					$this->generateUrl('_admin_biFolder_editGet', array('uid' => $biFolder->getId()))
				);
			} else {
				$this->flashMsgSession(
					'error',
					$this->translate('BiFolder.add.failure')
				);
			}
		}
		$this->gvars['BiFolderNewForm'] = $biFolderNewForm->createView();
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $biFolder) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('BiFolder.delete.success', array('%biFolder%' => $biFolder->getTitle()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $biFolder) {
				...
				$biDocNewForm = $this->createForm(BiDocNewTForm::class, $biDoc, array('folder' => $biFolder));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.biFolder.edit', array('%biFolder%' => $biFolder->getTitle()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.biFolder.edit.txt', array('%biFolder%' => $biFolder->getTitle()));
			...
			if (null == $biFolder) {
				...
				if (isset($reqData['BiFolderUpdateTitleForm'])) {
					...
					if ($biFolderUpdateTitleForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('BiFolder.edit.success', array('%biFolder%' => $biFolder->getTitle()))
						);
						...
						$this->flashMsgSession(
							'error',
							$this->translate('BiFolder.edit.failure', array('%biFolder%' => $biFolder->getTitle()))
						);
					}
