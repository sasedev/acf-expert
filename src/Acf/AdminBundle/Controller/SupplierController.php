...
class SupplierController extends BaseController
{
		...
		try {
			...
			if (null == $supplier) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Supplier.delete.success', array('%supplier%' => $supplier->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $supplier) {
				...
				$supplierUpdateDocsForm = $this->createForm(SupplierUpdateDocsTForm::class, $supplier, array('company' => $supplier->getCompany()));
				...
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.supplier.edit.txt', array('%supplier%' => $supplier->getLabel()));
			...
			if (null == $supplier) {
				...
				if (isset($reqData['SupplierUpdateForm'])) {
					...
					if ($supplierUpdateForm->isValid()) {
						...
						$this->flashMsgSession('error', $this->translate('Supplier.edit.failure', array('%supplier%' => $supplier->getLabel())));
					}
	...
	protected function traceEntity(Supplier $cloneSupplier, Supplier $supplier) {
		...
		$trace->setCompanyId($supplier->getCompany()->getId());
		...
		$trace->setActionId2($supplier->getCompany()->getId());
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Supplier.traceEdit',
					array('%supplier%' => $supplier->getLabel(), '%company%' => $supplier->getCompany()->getCorporateName())
					).$msg
				);
