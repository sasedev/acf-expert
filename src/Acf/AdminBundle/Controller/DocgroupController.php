		...
		try {
			...
			if (null == $docgroup) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Docgroup.delete.success', array('%docgroup%' => $docgroup->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroup) {
				...
				$docgroupUpdateParentForm = $this->createForm(DocgroupUpdateParentTForm::class, $docgroup, array('selfUrl' => $docgroup->getPageUrlFull(), 'company' => $docgroup->getCompany()));
				$docgroupUpdateDocsForm = $this->createForm(DocgroupUpdateDocsTForm::class, $docgroup, array('company' => $docgroup->getCompany()));
				...
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroup.edit.txt', array('%docgroup%' => $docgroup->getLabel()));
			...
			if (null == $docgroup) {
				...
				if (isset($reqData['DocgroupUpdateLabelForm'])) {
					...
					if ($docgroupUpdateLabelForm->isValid()) {
						...
						$this->flashMsgSession('error', $this->translate('Docgroup.edit.failure', array('%docgroup%' => $docgroup->getLabel())));
					}
	...
	protected function traceEntity(Docgroup $cloneDocgroup, Docgroup $docgroup) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Docgroup.traceEdit',
					array('%docgroup%' => $docgroup->getLabel(), '%company%' => $docgroup->getCompany()->getCorporateName())
					).$msg
				);
