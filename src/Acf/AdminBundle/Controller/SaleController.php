		...
		try {
			...
			if (null == $sale) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Sale.delete.success', array('%sale%' => $sale->getNumber()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $sale) {
				...
				$saleUpdateRelationForm = $this->createForm(SaleUpdateRelationTForm::class, $sale, array('monthlybalance' => $sale->getMonthlyBalance()));
				...
				$saleUpdateWithholdingForm = $this->createForm(SaleUpdateWithholdingTForm::class, $sale, array('monthlybalance' => $sale->getMonthlyBalance()));
				...
				$saleUpdateAccountForm = $this->createForm(SaleUpdateAccountTForm::class, $sale, array('monthlybalance' => $sale->getMonthlyBalance()));
			...
			if (null == $sale) {
				...
				if (isset($reqData['SaleUpdateNumberForm'])) {
					...
					if ($saleUpdateNumberForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Sale.edit.success', array('%sale%' => $sale->getNumber()))
						);
					...
					if ($secondaryVatNewForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('SecondaryVat.add.success', array('%sale%' => $sale->getNumber(), '%secondaryVat%' => $this->translate('SecondaryVat.vatInfo.'.$secondaryVat->getVatInfo())))
						);
	...
	protected function traceEntity(Sale $cloneSale, Sale $sale) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Sale.traceEdit',
					array('%sale%' => $sale->getLabel(), '%mbsale%' => $sale->getMonthlyBalance()->getRef(), '%company%' => $sale->getCompany()->getCorporateName())
					).$msg
				);
