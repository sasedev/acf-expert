...
class DocgroupcomptableController extends BaseController
{
	...
	public function addDocAction($uid)
	{
		...
		return $this->redirect($this->generateUrl('_admin_docgroupcomptable_editGet', array(
			'uid' => $uid
		)));
	}
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupcomptable) {
				...
				$this->flashMsgSession('success', $this->translate('Docgroupcomptable.delete.success', array(
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupcomptable) {
				...
				$traces = $em->getRepository('AcfDataBundle:Trace')->getAllByEntityId($docgroupcomptable->getId(), Trace::AE_DOCGROUPCOMPTABLE);
				...
				$docgroupcomptableUpdateOtherInfosForm = $this->createForm(DocgroupcomptableUpdateOtherInfosTForm::class, $docgroupcomptable);
				$docgroupcomptableUpdateParentForm = $this->createForm(DocgroupcomptableUpdateParentTForm::class, $docgroupcomptable, array(
					'selfUrl' => $docgroupcomptable->getPageUrlFull(),
					'company' => $docgroupcomptable->getCompany()
				));
				$docgroupcomptableUpdateDocsForm = $this->createForm(DocgroupcomptableUpdateDocsTForm::class, $docgroupcomptable, array(
					'company' => $docgroupcomptable->getCompany()
				));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupcomptable.edit', array(
					'%docgroupcomptable%' => $docgroupcomptable->getLabel()
				));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupcomptable.edit.txt', array(
					'%docgroupcomptable%' => $docgroupcomptable->getLabel()
				));
			...
			if (null == $docgroupcomptable) {
				...
				if (isset($reqData['DocgroupcomptableUpdateLabelForm'])) {
					...
					if ($docgroupcomptableUpdateLabelForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success', $this->translate('Docgroupcomptable.edit.success', array(
							'%docgroupcomptable%' => $docgroupcomptable->getLabel()
						)));
						...
						$this->flashMsgSession('error', $this->translate('Docgroupcomptable.edit.failure', array(
							'%docgroupcomptable%' => $docgroupcomptable->getLabel()
						)));
					}
	...
	protected function traceEntity(Docgroupcomptable $cloneDocgroupcomptable, Docgroupcomptable $docgroupcomptable)
	{
		...
		if ($msg != '') {
			...
			$trace->setMsg($this->translate('Docgroupcomptable.traceEdit', array(
				'%docgroupcomptable%' => $docgroupcomptable->getLabel(),
				'%company%' => $docgroupcomptable->getCompany()->getCorporateName()
			)) . $msg);
