		...
		try {
			...
			if (null == $mbsale) {
				...
				$traces = $em->getRepository('AcfDataBundle:Trace')
					->getAllByEntityId($mbsale->getId(), Trace::AE_MBSALE);
					...
					if ($saleImportForm->isValid()) {
						...
						for ($row = 1; $row <= $highestRow; $row ++) {
							...
							$bill = \trim(\strval($worksheet->getCellByColumnAndRow(2, $row)
								->getValue()));
							$customerNum = \intval($worksheet->getCellByColumnAndRow(4, $row)
								->getValue());
							...
							$balanceTtc = \floatval($worksheet->getCellByColumnAndRow(9, $row)
								->getValue());
							$vatInfo = \trim(\strval($worksheet->getCellByColumnAndRow(10, $row)
								->getValue()));
							$regime = \trim(\strval($worksheet->getCellByColumnAndRow(11, $row)
								->getValue()));
							$withholdingValue = \trim(\strval($worksheet->getCellByColumnAndRow(13, $row)
								->getValue() * 100));
							$balanceNet = \floatval($worksheet->getCellByColumnAndRow(14, $row)
								->getValue());
							$paymentType = \trim(\strval($worksheet->getCellByColumnAndRow(15, $row)
								->getValue()));
							$dtPayment = \PHPExcel_Shared_Date::ExcelToPHPObject(
								$worksheet->getCellByColumnAndRow(16, $row)
									->getValue());
							$accountLabel = \trim(\strval($worksheet->getCellByColumnAndRow(17, $row)
								->getValue()));
	...
	public function excelAction($uid)
	{
		...
		try {
			...
			foreach ($sales as $sale) {
				...
				$numb = $customersPrefix . $sale->getRelation()
					->getNumberFormated();
