		...
		try {
			...
			if (null == $bulletinInfo) {
				...
				$this->flashMsgSession('success',
					$this->translate('BulletinInfo.delete.success', array('%bulletinInfo%' => $bulletinInfo->getNum())));
			}
		} catch (\Exception $e) {
			$logger = $this->getLogger();
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $bulletinInfo) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.bulletinInfo.edit',
					array('%bulletinInfo%' => $bulletinInfo->getNum()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.bulletinInfo.edit.txt',
					array('%bulletinInfo%' => $bulletinInfo->getNum()));
			...
			if (null == $bulletinInfo) {
				...
				if (isset($reqData['BulletinInfoUpdateDtStartForm'])) {
					...
					if ($bulletinInfoUpdateDtStartForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('BulletinInfo.edit.success', array('%bulletinInfo%' => $bulletinInfo->getNum())));
						...
						$this->flashMsgSession('error',
							$this->translate('BulletinInfo.edit.failure', array('%bulletinInfo%' => $bulletinInfo->getNum())));
					}
					...
					if ($bulletinInfoTitleNewForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('BulletinInfoTitle.add.success', array('%bulletinInfoTitle%' => $bulletinInfoTitle->getTitle())));
						...
						$this->flashMsgSession('error',
							$this->translate('BulletinInfoTitle.add.failure'));
					}
				}
