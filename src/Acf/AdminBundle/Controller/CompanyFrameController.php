...
use Acf\AdminBundle\Form\CompanyFrame\UpdateTForm as CompanyFrameUpdateTForm;
...
class CompanyFrameController extends BaseController
{
    ...
    public function deleteAction($uid)
    {
        ...
        if (null == $urlFrom || trim($urlFrom) == '') {
            $urlFrom = $this->generateUrl('_admin_companyFrame_list');
        }
        ...
        try {
            ...
            if (null == $companyFrame) {
                ...
                $this->flashMsgSession('success', $this->translate('CompanyFrame.delete.success', array(
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $companyFrame) {
                ...
                $traces = $em->getRepository('AcfDataBundle:Trace')->getAllByEntityId($companyFrame->getId(), Trace::AE_FRAME);
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.companyFrame.edit', array(
                    '%companyFrame%' => $companyFrame->getFullName()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.companyFrame.edit.txt', array(
                    '%companyFrame%' => $companyFrame->getFullName()
                ));

                return $this->renderResponse('AcfAdminBundle:CompanyFrame:edit.html.twig', $this->gvars);
            }
        } catch (\Exception $e) {
            $logger = $this->getLogger();
            $logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
            ...
            if (null == $companyFrame) {
                ...
                if (isset($reqData['CompanyFrameUpdateForm'])) {
                    ...
                    if ($companyFrameUpdateForm->isValid()) {
                        ...
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('CompanyFrame.edit.success', array(
                            '%companyFrame%' => $companyFrame->getFullName()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('CompanyFrame.edit.failure', array(
                            '%companyFrame%' => $companyFrame->getFullName()
                        )));
                    }
                }
    ...
    protected function traceEntity(CompanyFrame $cloneCompanyFrame, CompanyFrame $companyFrame)
    {
        ...
        $trace->setCompanyId($companyFrame->getCompany()
            ->getId());
        ...
        $trace->setActionId2($companyFrame->getCompany()
            ->getId());
        ...
        if ($msg != '') {
            ...
            $trace->setMsg($this->translate('CompanyFrame.traceEdit', array(
                '%companyFrame%' => $companyFrame->getFullName(),
