...
use Acf\AdminBundle\Form\SecondaryVat\UpdateVatTForm as SecondaryVatUpdateVatTForm;
use Acf\AdminBundle\Form\SecondaryVat\UpdateBalanceTtcTForm as SecondaryVatUpdateBalanceTtcTForm;
use Acf\AdminBundle\Form\SecondaryVat\UpdateVatInfoTForm as SecondaryVatUpdateVatInfoTForm;
use Acf\AdminBundle\Form\SecondaryVat\UpdateBalanceNetTForm as SecondaryVatUpdateBalanceNetTForm;
...
class SecondaryVatController extends BaseController
{
		...
		try {
			...
			if (null == $secondaryVat) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('SecondaryVat.delete.success', array('%sale%' => $secondaryVat->getSale()->getLabel(), '%secondaryVat%' => $this->translate('SecondaryVat.vatInfo.'.$secondaryVat->getVatInfo()) ))
				);
			}
		} catch (\Exception $e) {
			...
			if (null == $secondaryVat) {
				...
				if (isset($reqData['SecondaryVatUpdateVatInfoForm'])) {
					...
					if ($secondaryVatUpdateVatInfoForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('SecondaryVat.edit.success', array('%sale%' => $secondaryVat->getSale()->getLabel(), '%secondaryVat%' => $this->translate('SecondaryVat.vatInfo.'.$secondaryVat->getVatInfo())))
						);
					...
					if ($secondaryVatUpdateVatForm->isValid()) {
						//$secondaryVat->setConversionRate(1);
						//$secondaryVat->setVatDevise($secondaryVat->getVat());
						$em->persist($secondaryVat);
						$em->flush();
						$this->flashMsgSession(
							'success',
					...
					if ($secondaryVatUpdateBalanceTtcForm->isValid()) {
						//$secondaryVat->setConversionRate(1);
						//$secondaryVat->setBalanceTtcDevise($secondaryVat->getBalanceTtc());
						$em->persist($secondaryVat);
						$em->flush();
						$this->flashMsgSession(
							'success',
					...
					if ($secondaryVatUpdateBalanceNetForm->isValid()) {
						//$secondaryVat->setConversionRate(1);
						//$secondaryVat->setBalanceNetDevise($secondaryVat->getBalanceNet());
						$em->persist($secondaryVat);
						$em->flush();
						$this->flashMsgSession(
							'success',
	...
	protected function traceEntity(SecondaryVat $cloneSecondaryVat, SecondaryVat $secondaryVat) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'SecondaryVat.traceEdit',
					array('%secondaryVat%' => $this->translate('SecondaryVat.vatInfo.'.$secondaryVat->getVatInfo()), '%sale%' => $secondaryVat->getSale()->getLabel(), '%mbsale%' => $secondaryVat->getSale()->getMonthlyBalance()->getRef(), '%company%' => $secondaryVat->getSale()->getMonthlyBalance()->getCompany()->getCorporateName())
					).$msg
				);
