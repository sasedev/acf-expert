...
class CompanyUserController extends BaseController
{
		...
		try {
			...
			if (null == $companyUser) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('CompanyUser.delete.success', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $companyUser) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.companyUser.edit', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.companyUser.edit.txt', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName()));
			...
			if (null == $companyUser) {
				...
				if (isset($reqData['CompanyUserUpdateForm'])) {
					...
					$user = $companyUser->getUser();
						...
						$this->flashMsgSession(
							'success',
							$this->translate('CompanyUser.edit.success', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName()))
						);
						...
						$this->flashMsgSession('error', $this->translate('CompanyUser.edit.failure', array('%company%' => $companyUser->getCompany()->getCorporateName(), '%user%' => $companyUser->getUser()->getFullName())));
					}
				}
	...
	protected function traceEntity(CompanyUser $cloneCompanyUser, CompanyUser $companyUser) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'CompanyUser.traceEdit',
					array('%user%' => $companyUser->getUser()->getFullName(), '%company%' => $companyUser->getCompany()->getCorporateName())
					).$msg
				);
