...
class CompanyLabelController extends BaseController
{
		...
		try {
			...
			if (null == $companyLabel) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('CompanyLabel.delete.success', array('%companyLabel%' => $companyLabel->getName()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $companyLabel) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.companyLabel.edit', array('%companyLabel%' => $companyLabel->getName()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.companyLabel.edit.txt', array('%companyLabel%' => $companyLabel->getName()));
			...
			if (null == $companyLabel) {
				...
				if (isset($reqData['CompanyLabelUpdateForm'])) {
					...
					if ($companyLabelUpdateForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('CompanyLabel.edit.success', array('%companyLabel%' => $companyLabel->getName()))
						);
						...
						$this->flashMsgSession('error', $this->translate('CompanyLabel.edit.failure', array('%companyLabel%' => $companyLabel->getName())));
					}
				}
	...
	protected function traceEntity(CompanyLabel $cloneCompanyLabel, CompanyLabel $companyLabel) {
		...
		$trace->setCompanyId($companyLabel->getCompany()->getId());
		...
		$trace->setActionId2($companyLabel->getCompany()->getId());
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'CompanyLabel.traceEdit',
					array('%companyLabel%' => $companyLabel->getName(), '%company%' => $companyLabel->getCompany()->getCorporateName())
					).$msg
				);
