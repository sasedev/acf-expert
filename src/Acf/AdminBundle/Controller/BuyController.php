...
use Acf\AdminBundle\Form\Buy\UpdateNumberTForm as BuyUpdateNumberTForm;
use Acf\AdminBundle\Form\Buy\UpdateDtActivationTForm as BuyUpdateDtActivationTForm;
use Acf\AdminBundle\Form\Buy\UpdateBillTForm as BuyUpdateBillTForm;
use Acf\AdminBundle\Form\Buy\UpdateRelationTForm as BuyUpdateRelationTForm;
use Acf\AdminBundle\Form\Buy\UpdateLabelTForm as BuyUpdateLabelTForm;
use Acf\AdminBundle\Form\Buy\UpdateDeviseTForm as BuyUpdateDeviseTForm;
use Acf\AdminBundle\Form\Buy\UpdateConversionRateTForm as BuyUpdateConversionRateTForm;
use Acf\AdminBundle\Form\Buy\UpdateVatTForm as BuyUpdateVatTForm;
use Acf\AdminBundle\Form\Buy\UpdateVatDeviseTForm as BuyUpdateVatDeviseTForm;
use Acf\AdminBundle\Form\Buy\UpdateStampTForm as BuyUpdateStampTForm;
use Acf\AdminBundle\Form\Buy\UpdateStampDeviseTForm as BuyUpdateStampDeviseTForm;
use Acf\AdminBundle\Form\Buy\UpdateBalanceTtcTForm as BuyUpdateBalanceTtcTForm;
use Acf\AdminBundle\Form\Buy\UpdateBalanceTtcDeviseTForm as BuyUpdateBalanceTtcDeviseTForm;
use Acf\AdminBundle\Form\Buy\UpdateRegimeTForm as BuyUpdateRegimeTForm;
use Acf\AdminBundle\Form\Buy\UpdateWithholdingTForm as BuyUpdateWithholdingTForm;
use Acf\AdminBundle\Form\Buy\UpdateNatureTForm as BuyUpdateNatureTForm;
use Acf\AdminBundle\Form\Buy\UpdateBalanceNetTForm as BuyUpdateBalanceNetTForm;
use Acf\AdminBundle\Form\Buy\UpdateBalanceNetDeviseTForm as BuyUpdateBalanceNetDeviseTForm;
use Acf\AdminBundle\Form\Buy\UpdatePaymentTypeTForm as BuyUpdatePaymentTypeTForm;
use Acf\AdminBundle\Form\Buy\UpdateTransactionStatusTForm as BuyUpdateTransactionStatusTForm;
use Acf\AdminBundle\Form\Buy\UpdateValidatedTForm as BuyUpdateValidatedTForm;
use Acf\AdminBundle\Form\Buy\UpdateAccountTForm as BuyUpdateAccountTForm;
use Acf\AdminBundle\Form\Buy\UpdateOtherInfosTForm as BuyUpdateOtherInfosTForm;
use Acf\AdminBundle\Form\Buy\UpdateDocsTForm as BuyUpdateDocsTForm;
...
class BuyController extends BaseController
{
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $buy) {
				...
				$this->flashMsgSession('success', $this->translate('Buy.delete.success', array(
					'%buy%' => $buy->getNumber()
				)));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $buy) {
				...
				$traces = $em->getRepository('AcfDataBundle:Trace')->getAllByEntityId($buy->getId(), Trace::AE_BUY);
				...
				$buyUpdateRelationForm = $this->createForm(BuyUpdateRelationTForm::class, $buy, array(
					'monthlybalance' => $buy->getMonthlyBalance()
				));
				...
				$buyUpdateWithholdingForm = $this->createForm(BuyUpdateWithholdingTForm::class, $buy, array(
					'monthlybalance' => $buy->getMonthlyBalance()
				));
				...
				$buyUpdateValidatedForm = $this->createForm(BuyUpdateValidatedTForm::class, $buy);
				$buyUpdateAccountForm = $this->createForm(BuyUpdateAccountTForm::class, $buy, array(
					'monthlybalance' => $buy->getMonthlyBalance()
				));
				$buyUpdateNatureForm = $this->createForm(BuyUpdateNatureTForm::class, $buy, array(
					'monthlybalance' => $buy->getMonthlyBalance()
				));
				$buyUpdateOtherInfosForm = $this->createForm(BuyUpdateOtherInfosTForm::class, $buy);
				$buyUpdateDocsForm = $this->createForm(BuyUpdateDocsTForm::class, $buy, array(
				...
				$this->gvars['BuyUpdateValidatedForm'] = $buyUpdateValidatedForm->createView();
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.buy.edit', array(
					'%buy%' => $buy->getNumber()
				));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.buy.edit.txt', array(
					'%buy%' => $buy->getNumber()
				));

				return $this->renderResponse('AcfAdminBundle:Buy:edit.html.twig', $this->gvars);
			}
		} catch (\Exception $e) {
			$logger = $this->getLogger();
			$logger->addCritical($e->getLine() . ' ' . $e->getMessage() . ' ' . $e->getTraceAsString());
			...
			if (null == $buy) {
				...
				if (isset($reqData['BuyUpdateNumberForm'])) {
					...
					if ($buyUpdateNumberForm->isValid()) {
						$em->persist($buy);
						$em->flush();
						$this->flashMsgSession('success', $this->translate('Buy.edit.success', array(
							'%buy%' => $buy->getNumber()
						)));
						...
						$this->flashMsgSession('error', $this->translate('Buy.edit.failure', array(
							'%buy%' => $buy->getNumber()
						)));
					}
				...
				} elseif (isset($reqData['BuyUpdateValidatedForm'])) {
					$this->gvars['tabActive'] = 2;
					$this->getSession()->set('tabActive', 2);
					$buyUpdateValidatedForm->handleRequest($request);
					if ($buyUpdateValidatedForm->isValid()) {
						$em->persist($buy);
						$em->flush();
						$this->flashMsgSession('success', $this->translate('Buy.edit.success', array(
							'%buy%' => $buy->getNumber()
	...
	protected function traceEntity(Buy $cloneBuy, Buy $buy)
	{
		...
		if ($msg != '') {
			...
			$trace->setMsg($this->translate('Buy.traceEdit', array(
				'%buy%' => $buy->getLabel(),
				'%mbpurchase%' => $buy->getMonthlyBalance()->getRef(),
				'%company%' => $buy->getCompany()->getCorporateName()
			)) . $msg);
