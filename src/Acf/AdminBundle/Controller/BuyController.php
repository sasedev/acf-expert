		...
		try {
			...
			if (null == $buy) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Buy.delete.success', array('%buy%' => $buy->getNumber()))
				);
			}
		} catch (\Exception $e) {
	...
	protected function traceEntity(Buy $cloneBuy, Buy $buy) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Buy.traceEdit',
					array('%buy%' => $buy->getLabel(), '%mbpurchase%' => $buy->getMonthlyBalance()->getRef(), '%company%' => $buy->getCompany()->getCorporateName())
					).$msg
				);
