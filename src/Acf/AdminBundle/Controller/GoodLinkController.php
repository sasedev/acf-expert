    ...
    public function addPostAction()
    {
        ...
        if (isset($reqData['GoodLinkNewForm'])) {
            ...
            if ($goodLinkNewForm->isValid()) {
                ...
                $em->persist($goodLink);
                $em->flush();
                $this->flashMsgSession('success', $this->translate('GoodLink.add.success', array(
                    '%goodLink%' => $goodLink->getUrl()
                )));

                return $this->redirect($this->generateUrl('_admin_goodLink_editGet', array(
                    'uid' => $goodLink->getId()
                )));
            } else {
                $this->flashMsgSession('error', $this->translate('GoodLink.add.failure'));
            }
        }
        $this->gvars['GoodLinkNewForm'] = $goodLinkNewForm->createView();
    ...
    public function deleteAction($uid)
    {
        ...
        try {
            ...
            if (null == $goodLink) {
                ...
                $this->flashMsgSession('success', $this->translate('GoodLink.delete.success', array(
                    '%goodLink%' => $goodLink->getUrl()
                )));
            }
        } catch (\Exception $e) {
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $goodLink) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.goodLink.edit', array(
                    '%goodLink%' => $goodLink->getUrl()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.goodLink.edit.txt', array(
                    '%goodLink%' => $goodLink->getUrl()
                ));
            ...
            if (null == $goodLink) {
                ...
                if (isset($reqData['GoodLinkUpdateUrlForm'])) {
                    ...
                    if ($goodLinkUpdateUrlForm->isValid()) {
                        $em->persist($goodLink);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('GoodLink.edit.success', array(
                            '%goodLink%' => $goodLink->getUrl()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('GoodLink.edit.failure', array(
                            '%goodLink%' => $goodLink->getUrl()
                        )));
                    }
