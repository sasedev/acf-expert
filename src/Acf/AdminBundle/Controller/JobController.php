		...
		if (isset($reqData['JobNewForm'])) {
			...
			if ($jobNewForm->isValid()) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Job.add.success', array('%job%' => $job->getLabel()))
				);
				...
				$this->flashMsgSession(
					'error',
					$this->translate('Job.add.failure')
				);
			}
		}
		$this->gvars['job'] = $job;
	...
	public function deleteAction($id)
	{
		...
		try {
			...
			if (null == $job) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Job.delete.success', array('%job%' => $job->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($id)
	{
		...
		try {
			...
			if (null == $job) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.job.edit', array('%job%' => $job->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.job.edit.txt', array('%job%' => $job->getLabel()));
			...
			if (null == $job) {
				...
				if (isset($reqData['JobUpdateForm'])) {
					...
					if ($jobUpdateForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Job.edit.success', array('%job%' => $job->getLabel()))
						);
						...
						$this->flashMsgSession(
							'error',
							$this->translate('Job.edit.failure', array('%job%' => $job->getLabel()))
						);
					}
				}
