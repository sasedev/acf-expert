    ...
    public function addPostAction()
    {
        ...
        if (isset($reqData['RoleNewForm'])) {
            ...
            if ($roleNewForm->isValid()) {
                ...
                $em->persist($role);
                $em->flush();
                $this->flashMsgSession('success', $this->translate('Role.add.success', array(
                    '%role%' => $role->getName()
                )));

                return $this->redirect($this->generateUrl('_admin_role_editGet', array(
                    'id' => $role->getId()
                )));
            } else {
                $this->flashMsgSession('error', $this->translate('Role.add.failure'));
            }
        }
        $this->gvars['RoleNewForm'] = $roleNewForm->createView();
        $this->gvars['role'] = $role;
    ...
    public function deleteAction($id)
    {
        ...
        try {
            ...
            if (null == $role) {
                ...
                $this->flashMsgSession('success', $this->translate('Role.delete.success', array(
                    '%role%' => $role->getName()
                )));
            }
        } catch (\Exception $e) {
    ...
    public function editGetAction($id)
    {
        ...
        try {
            ...
            if (null == $role) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.role.edit', array(
                    '%role%' => $role->getName()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.role.edit.txt', array(
                    '%role%' => $role->getName()
                ));
            ...
            if (null == $role) {
                ...
                if (isset($reqData['RoleUpdateDescriptionForm'])) {
                    ...
                    if ($roleUpdateDescriptionForm->isValid()) {
                        $em->persist($role);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('Role.edit.success', array(
                            '%role%' => $role->getName()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('Role.edit.failure', array(
                            '%role%' => $role->getName()
                        )));
                    }
