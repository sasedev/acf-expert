		...
		if (isset($reqData['FeedReadNewForm'])) {
			...
			if ($feedReadNewForm->isValid()) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('FeedRead.add.success', array('%feedRead%' => $feedRead->getUrl()))
				);

				return $this->redirect(
					$this->generateUrl('_admin_feedRead_editGet', array('uid' => $feedRead->getId()))
				);
			} else {
				$this->flashMsgSession(
					'error',
					$this->translate('FeedRead.add.failure')
				);
			}
		}
		$this->gvars['FeedReadNewForm'] = $feedReadNewForm->createView();
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $feedRead) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('FeedRead.delete.success', array('%feedRead%' => $feedRead->getUrl()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $feedRead) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.feedRead.edit', array('%feedRead%' => $feedRead->getUrl()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.feedRead.edit.txt', array('%feedRead%' => $feedRead->getUrl()));
			...
			if (null == $feedRead) {
				...
				if (isset($reqData['FeedReadUpdateUrlForm'])) {
					...
					if ($feedReadUpdateUrlForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('FeedRead.edit.success', array('%feedRead%' => $feedRead->getUrl()))
						);
						...
						$this->flashMsgSession(
							'error',
							$this->translate('FeedRead.edit.failure', array('%feedRead%' => $feedRead->getUrl()))
						);
					}
