...
class DocgroupsystController extends BaseController
{
	...
	public function addDocAction($uid)
	{
		...
		return $this->redirect($this->generateUrl('_admin_docgroupsyst_editGet', array(
			'uid' => $uid
		)));
	}
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupsyst) {
				...
				$this->flashMsgSession('success', $this->translate('Docgroupsyst.delete.success', array(
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupsyst) {
				...
				$docgroupsystUpdateParentForm = $this->createForm(DocgroupsystUpdateParentTForm::class, $docgroupsyst, array(
					'selfUrl' => $docgroupsyst->getPageUrlFull(),
					'company' => $docgroupsyst->getCompany()
				));
				$docgroupsystUpdateDocsForm = $this->createForm(DocgroupsystUpdateDocsTForm::class, $docgroupsyst, array(
					'company' => $docgroupsyst->getCompany()
				));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupsyst.edit', array(
					'%docgroupsyst%' => $docgroupsyst->getLabel()
				));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupsyst.edit.txt', array(
					'%docgroupsyst%' => $docgroupsyst->getLabel()
				));
			...
			if (null == $docgroupsyst) {
				...
				if (isset($reqData['DocgroupsystUpdateLabelForm'])) {
					...
					if ($docgroupsystUpdateLabelForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success', $this->translate('Docgroupsyst.edit.success', array(
							'%docgroupsyst%' => $docgroupsyst->getLabel()
						)));
						...
						$this->flashMsgSession('error', $this->translate('Docgroupsyst.edit.failure', array(
							'%docgroupsyst%' => $docgroupsyst->getLabel()
						)));
					}
	...
	protected function traceEntity(Docgroupsyst $cloneDocgroupsyst, Docgroupsyst $docgroupsyst)
	{
		...
		if ($msg != '') {
			...
			$trace->setMsg($this->translate('Docgroupsyst.traceEdit', array(
				'%docgroupsyst%' => $docgroupsyst->getLabel(),
				'%company%' => $docgroupsyst->getCompany()->getCorporateName()
			)) . $msg);
