...
class DocgroupbankController extends BaseController
{
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupbank) {
				...
				$this->flashMsgSession('success', $this->translate('Docgroupbank.delete.success', array(
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupbank) {
				...
				$docgroupbankUpdateParentForm = $this->createForm(DocgroupbankUpdateParentTForm::class, $docgroupbank, array(
					'selfUrl' => $docgroupbank->getPageUrlFull(),
					'company' => $docgroupbank->getCompany()
				));
				$docgroupbankUpdateDocsForm = $this->createForm(DocgroupbankUpdateDocsTForm::class, $docgroupbank, array(
					'company' => $docgroupbank->getCompany()
				));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupbank.edit', array(
					'%docgroupbank%' => $docgroupbank->getLabel()
				));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupbank.edit.txt', array(
					'%docgroupbank%' => $docgroupbank->getLabel()
				));
			...
			if (null == $docgroupbank) {
				...
				if (isset($reqData['DocgroupbankUpdateLabelForm'])) {
					...
					if ($docgroupbankUpdateLabelForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success', $this->translate('Docgroupbank.edit.success', array(
							'%docgroupbank%' => $docgroupbank->getLabel()
						)));
						...
						$this->flashMsgSession('error', $this->translate('Docgroupbank.edit.failure', array(
							'%docgroupbank%' => $docgroupbank->getLabel()
						)));
					}
	...
	protected function traceEntity(Docgroupbank $cloneDocgroupbank, Docgroupbank $docgroupbank)
	{
		...
		if ($msg != '') {
			...
			$trace->setMsg($this->translate('Docgroupbank.traceEdit', array(
				'%docgroupbank%' => $docgroupbank->getLabel(),
				'%company%' => $docgroupbank->getCompany()->getCorporateName()
			)) . $msg);
