		...
		if (isset($reqData['GoodDocNewForm'])) {
			...
			if ($goodDocNewForm->isValid()) {
				...
				$fileName = sha1(uniqid(mt_rand(), true)) . '.' .
					strtolower($goodDocFile->getClientOriginalExtension());
				...
				$this->flashMsgSession(
					'success',
					$this->translate('GoodDoc.add.success', array('%goodDoc%' => $goodDoc->getTitle()))
				);

				return $this->redirect(
					$this->generateUrl('_admin_goodDoc_editGet', array('uid' => $goodDoc->getId()))
				);
			} else {
				$this->flashMsgSession(
					'error',
					$this->translate('GoodDoc.add.failure')
				);
			}
		}
		$this->gvars['GoodDocNewForm'] = $goodDocNewForm->createView();
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $goodDoc) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('GoodDoc.delete.success', array('%goodDoc%' => $goodDoc->getTitle()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $goodDoc) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.goodDoc.edit', array('%goodDoc%' => $goodDoc->getTitle()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.goodDoc.edit.txt', array('%goodDoc%' => $goodDoc->getTitle()));
			...
			if (null == $goodDoc) {
				...
				if (isset($reqData['GoodDocUpdateDescriptionForm'])) {
					...
					if ($goodDocUpdateDescriptionForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('GoodDoc.edit.success', array('%goodDoc%' => $goodDoc->getTitle()))
						);
						...
						$this->flashMsgSession(
							'error',
							$this->translate('GoodDoc.edit.failure', array('%goodDoc%' => $goodDoc->getTitle()))
						);
					}
