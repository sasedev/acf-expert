...
class ShareholderController extends BaseController
{
		...
		try {
			...
			if (null == $shareholder) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Shareholder.delete.success', array('%shareholder%' => $shareholder->getName()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $shareholder) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.shareholder.edit', array('%shareholder%' => $shareholder->getName()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.shareholder.edit.txt', array('%shareholder%' => $shareholder->getName()));
			...
			if (null == $shareholder) {
				...
				if (isset($reqData['ShareholderUpdateForm'])) {
					...
					if ($shareholderUpdateForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Shareholder.edit.success', array('%shareholder%' => $shareholder->getName()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Shareholder.edit.failure', array('%shareholder%' => $shareholder->getName())));
					}
				}
	...
	protected function traceEntity(Shareholder $cloneShareholder, Shareholder $shareholder) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Shareholder.traceEdit',
					array('%shareholder%' => $shareholder->getName(), '%company%' => $shareholder->getCompany()->getCorporateName())
					).$msg
				);
