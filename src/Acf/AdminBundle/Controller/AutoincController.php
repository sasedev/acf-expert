    ...
    public function addPostAction()
    {
        ...
        if (isset($reqData['AutoincNewForm'])) {
            ...
            if ($autoincNewForm->isValid()) {
                ...
                $em->persist($autoinc);
                $em->flush();
                $this->flashMsgSession('success', $this->translate('Autoinc.add.success', array(
                    '%autoinc%' => $autoinc->getName()
                )));

                return $this->redirect($this->generateUrl('_admin_autoinc_editGet', array(
                    'uid' => $autoinc->getId()
                )));
            } else {
                $this->flashMsgSession('error', $this->translate('Autoinc.add.failure'));
            }
        }
        $this->gvars['AutoincNewForm'] = $autoincNewForm->createView();
    ...
    public function deleteAction($uid)
    {
        ...
        try {
            ...
            if (null == $autoinc) {
                ...
                $this->flashMsgSession('success', $this->translate('Autoinc.delete.success', array(
                    '%autoinc%' => $autoinc->getName()
                )));
            }
        } catch (\Exception $e) {
    ...
    public function editGetAction($uid)
    {
        ...
        try {
            ...
            if (null == $autoinc) {
                ...
                $this->gvars['pagetitle'] = $this->translate('pagetitle.autoinc.edit', array(
                    '%autoinc%' => $autoinc->getName()
                ));
                $this->gvars['pagetitle_txt'] = $this->translate('pagetitle.autoinc.edit.txt', array(
                    '%autoinc%' => $autoinc->getName()
                ));
            ...
            if (null == $autoinc) {
                ...
                if (isset($reqData['AutoincUpdateForm'])) {
                    ...
                    if ($autoincUpdateForm->isValid()) {
                        $em->persist($autoinc);
                        $em->flush();
                        $this->flashMsgSession('success', $this->translate('Autoinc.edit.success', array(
                            '%autoinc%' => $autoinc->getName()
                        )));
                        ...
                        $this->flashMsgSession('error', $this->translate('Autoinc.edit.failure', array(
                            '%autoinc%' => $autoinc->getName()
                        )));
                    }
                }
