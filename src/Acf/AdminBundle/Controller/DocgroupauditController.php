...
class DocgroupauditController extends BaseController
{
		...
		try {
			...
			if (null == $docgroupaudit) {
				...
				$this->flashMsgSession('success',
					$this->translate('Docgroupaudit.delete.success', array('%docgroupaudit%' => $docgroupaudit->getLabel())));
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $docgroupaudit) {
				...
				$docgroupauditUpdateParentForm = $this->createForm(DocgroupauditUpdateParentTForm::class, $docgroupaudit,
					array('selfUrl' => $docgroupaudit->getPageUrlFull(), 'company' => $docgroupaudit->getCompany()));
				$docgroupauditUpdateDocsForm = $this->createForm(DocgroupauditUpdateDocsTForm::class, $docgroupaudit,
					array('company' => $docgroupaudit->getCompany()));

				$doc = new Doc();
				$doc->setCompany($docgroupaudit->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $docgroupaudit->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.docgroupaudit.edit',
					array('%docgroupaudit%' => $docgroupaudit->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.docgroupaudit.edit.txt',
					array('%docgroupaudit%' => $docgroupaudit->getLabel()));
			...
			if (null == $docgroupaudit) {
				...
				if (isset($reqData['DocgroupauditUpdateLabelForm'])) {
					...
					if ($docgroupauditUpdateLabelForm->isValid()) {
						...
						$em->flush();
						$this->flashMsgSession('success',
							$this->translate('Docgroupaudit.edit.success', array('%docgroupaudit%' => $docgroupaudit->getLabel())));
						...
						$this->flashMsgSession('error',
							$this->translate('Docgroupaudit.edit.failure', array('%docgroupaudit%' => $docgroupaudit->getLabel())));
					}
	...
	protected function traceEntity(Docgroupaudit $cloneDocgroupaudit, Docgroupaudit $docgroupaudit)
	{
		...
		if ($cloneDocgroupaudit->getParent() != $docgroupaudit->getParent() &&
			(($cloneDocgroupaudit->getParent() == null && $docgroupaudit->getParent() != null) ||
			($cloneDocgroupaudit->getParent() != null && $docgroupaudit->getParent() == null) ||
			($docgroupaudit->getParent() != null && $cloneDocgroupaudit->getParent() != null &&
			$cloneDocgroupaudit->getParent()->getPageUrlFull() != $docgroupaudit->getParent()->getPageUrlFull()))) {
		...
		if (\count(\array_diff($docgroupaudit->getDocs()->toArray(), $cloneDocgroupaudit->getDocs()->toArray())) != 0 ||
			\count(\array_diff($cloneDocgroupaudit->getDocs()->toArray(), $docgroupaudit->getDocs()->toArray())) != 0) {
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate('Docgroupaudit.traceEdit',
					array('%docgroupaudit%' => $docgroupaudit->getLabel(), '%company%' => $docgroupaudit->getCompany()
