...
class BiDocController extends BaseController
{
	...
	public function listAction()
	{
		...
		$this->gvars['pagetitle'] = $this->translate('pagetitle.biDoc.list');
		$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.biDoc.list.txt');
		return $this->renderResponse('AcfAdminBundle:BiDoc:list.html.twig', $this->gvars);
	}
	...
	public function addGetAction()
	{
		...
		$biDoc = new BiDoc();
		$biDocNewForm = $this->createForm(BiDocNewTForm::class, $biDoc);
		$this->gvars['biDoc'] = $biDoc;
		$this->gvars['BiDocNewForm'] = $biDocNewForm->createView();

		$this->gvars['pagetitle'] = $this->translate('pagetitle.biDoc.add');
		$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.biDoc.add.txt');
		$this->gvars['smenu_active'] = 'add';

		return $this->renderResponse('AcfAdminBundle:BiDoc:add.html.twig', $this->gvars);
	}
	...
	public function addPostAction()
	{
		...
		if (null == $urlFrom || trim($urlFrom) == '') {
			return $this->redirect($this->generateUrl('_admin_biDoc_addGet'));
		}
		...
		if (isset($reqData['BiDocNewForm'])) {
			...
			if ($biDocNewForm->isValid()) {
				...
				$fileName = sha1(uniqid(mt_rand(), true)) . '.' .
					strtolower($biDocFile->getClientOriginalExtension());
				...
				$this->flashMsgSession(
					'success',
					$this->translate('BiDoc.add.success', array('%biDoc%' => $biDoc->getTitle()))
				);

				return $this->redirect(
					$this->generateUrl('_admin_biDoc_editGet', array('uid' => $biDoc->getId()))
				);
			} else {
				$this->flashMsgSession(
					'error',
					$this->translate('BiDoc.add.failure')
				);
			}
		}
		$this->gvars['BiDocNewForm'] = $biDocNewForm->createView();
	...
	public function deleteAction($uid)
	{
		...
		try {
			...
			if (null == $biDoc) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('BiDoc.delete.success', array('%biDoc%' => $biDoc->getTitle()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $biDoc) {
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.biDoc.edit', array('%biDoc%' => $biDoc->getTitle()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.biDoc.edit.txt', array('%biDoc%' => $biDoc->getTitle()));
			...
			if (null == $biDoc) {
				...
				if (isset($reqData['BiDocUpdateDescriptionForm'])) {
					...
					if ($biDocUpdateDescriptionForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('BiDoc.edit.success', array('%biDoc%' => $biDoc->getTitle()))
						);
						...
						$this->flashMsgSession(
							'error',
							$this->translate('BiDoc.edit.failure', array('%biDoc%' => $biDoc->getTitle()))
						);
					}
