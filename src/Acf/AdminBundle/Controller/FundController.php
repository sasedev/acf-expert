...
class FundController extends BaseController
{
		...
		try {
			...
			if (null == $fund) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Fund.delete.success', array('%fund%' => $fund->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $fund) {
				...
				$fundUpdateDocsForm = $this->createForm(FundUpdateDocsTForm::class, $fund, array('company' => $fund->getCompany()));

				$doc = new Doc();
				$doc->setCompany($fund->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $fund->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.fund.edit', array('%fund%' => $fund->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.fund.edit.txt', array('%fund%' => $fund->getLabel()));
			...
			if (null == $fund) {
				...
				if (isset($reqData['FundUpdateLabelForm'])) {
					...
					if ($fundUpdateLabelForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Fund.edit.success', array('%fund%' => $fund->getLabel()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Fund.edit.failure', array('%fund%' => $fund->getLabel())));
					}
	...
	protected function traceEntity(Fund $cloneFund, Fund $fund) {
		...
		$trace->setCompanyId($fund->getCompany()->getId());
		...
		$trace->setActionId2($fund->getCompany()->getId());
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Fund.traceEdit',
					array('%fund%' => $fund->getLabel(), '%company%' => $fund->getCompany()->getCorporateName())
					).$msg
				);
