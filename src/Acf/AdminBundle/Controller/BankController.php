...
class BankController extends BaseController
{
		...
		try {
			...
			if (null == $bank) {
				...
				$this->flashMsgSession(
					'success',
					$this->translate('Bank.delete.success', array('%bank%' => $bank->getLabel()))
				);
			}
		} catch (\Exception $e) {
	...
	public function editGetAction($uid)
	{
		...
		try {
			...
			if (null == $bank) {
				...
				$bankUpdateDocsForm = $this->createForm(BankUpdateDocsTForm::class, $bank, array('company' => $bank->getCompany()));

				$doc = new Doc();
				$doc->setCompany($bank->getCompany());
				$docNewForm = $this->createForm(DocNewTForm::class, $doc, array('company' => $bank->getCompany()));
				...
				$this->gvars['pagetitle'] = $this->translate('pagetitle.bank.edit', array('%bank%' => $bank->getLabel()));
				$this->gvars['pagetitle_txt'] = $this->translate('pagetitle.bank.edit.txt', array('%bank%' => $bank->getLabel()));
			...
			if (null == $bank) {
				...
				if (isset($reqData['BankUpdateLabelForm'])) {
					...
					if ($bankUpdateLabelForm->isValid()) {
						...
						$this->flashMsgSession(
							'success',
							$this->translate('Bank.edit.success', array('%bank%' => $bank->getLabel()))
						);
						...
						$this->flashMsgSession('error', $this->translate('Bank.edit.failure', array('%bank%' => $bank->getLabel())));
					}
	...
	protected function traceEntity(Bank $cloneBank, Bank $bank) {
		...
		$trace->setCompanyId($bank->getCompany()->getId());
		...
		$trace->setActionId2($bank->getCompany()->getId());
		...
		if ($msg != "") {
			...
			$trace->setMsg(
				$this->translate(
					'Bank.traceEdit',
					array('%bank%' => $bank->getLabel(), '%company%' => $bank->getCompany()->getCorporateName())
					).$msg
				);
