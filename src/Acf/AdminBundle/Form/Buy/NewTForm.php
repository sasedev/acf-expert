...
use Acf\AdminBundle\Form\Buy\Doc\BuyDocTForm;
...
class NewTForm extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options)
	{
		...
		if (null == $this->mbpurchase) {
			$builder->add('monthlyBalance', EntityType::class,
				array(
			...
			$mbpurchase_id = $this->mbpurchase->getId();
			$builder->add('monthlyBalance', EntityidType::class,
				array(
					'label' => 'Buy.monthlyBalance.label',
					'class' => 'AcfDataBundle:MBPurchase',
					'query_builder' => function (MBPurchaseRepository $mbsr) use ($mbpurchase_id)
					{
						return $mbsr->createQueryBuilder('mbs')
							->where('mbs.id = :id')
							->setParameter('id', $mbpurchase_id)
							->orderBy('mbs.ref', 'ASC');
					},
					'choice_label' => 'id',
					'multiple' => false,
		...
		$builder->add('dtActivation', DateType::class,
			array(
		...
		if (null == $this->mbpurchase) {
			...
			$company_id = $this->mbpurchase->getCompany()
				->getId();
			$builder->add('relation', EntityType::class,
				array(
					'label' => 'Buy.relation.label',
		...
		$builder->add('regime', ChoiceType::class,
			array(
		...
		if (null == $this->mbpurchase) {
			...
			$builder->add('nature', EntityType::class,
				array(
					'label' => 'Buy.nature.label',
					'class' => 'AcfDataBundle:CompanyNature',
					'query_builder' => function (CompanyNatureRepository $cnr) use ($company_id)
					{
						return $cnr->createQueryBuilder('cn')
							->join('cn.company', 'c')
							->where('c.id = :cid')
		...
		$builder->add('transactionStatus', ChoiceType::class,
			array(
		...
		$builder->add('dtPayment', DateType::class,
			array(
		...
		$builder->add('validated', ChoiceType::class,
			array(
				'label' => 'Buy.validated.label',
				'choices_as_values' => true,
				'choices' => Buy::choiceValidated(),
				'expanded' => true,
				'attr' => array(
					'choice_label_trans' => true
				)
