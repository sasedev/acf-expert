...
class UpdateParentTForm extends AbstractType
{
		...
		if (null == $this->company) {
			$builder->add('parent', EntityType::class,
				array('label' => 'Docgroup.parent.label', 'class' => 'AcfDataBundle:Docgroup',
					'query_builder' => function (DocgroupRepository $dgr) use ($selfUrl)
					{
			...
			$builder->add('parent', EntityType::class,
				array('label' => 'Docgroup.parent.label', 'class' => 'AcfDataBundle:Docgroup',
					'query_builder' => function (DocgroupRepository $dgr) use ($selfUrl, $company_id)
					{
						$qb = $dgr->createQueryBuilder('d')
							->join('d.company', 'c')
							->where('c.id = :cid')
	...
	{
		return 'DocgroupUpdateParentForm';
	}
