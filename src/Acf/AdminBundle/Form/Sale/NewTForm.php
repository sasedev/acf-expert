...
use Acf\AdminBundle\Form\Sale\Doc\SaleDocTForm;
use Acf\AdminBundle\Form\Sale\SecondaryVat\SaleSecondaryVatTForm;
...
class NewTForm extends AbstractType
{
	...
	public function buildForm(FormBuilderInterface $builder, array $options)
	{
		...
		if (null == $this->mbsale) {
			...
			$mbsale_id = $this->mbsale->getId();
			$builder->add('monthlyBalance', EntityidType::class,
				array(
					'label' => 'Sale.monthlyBalance.label',
					'class' => 'AcfDataBundle:MBSale',
					'query_builder' => function (MBSaleRepository $mbsr) use ($mbsale_id)
					{
						return $mbsr->createQueryBuilder('mbs')
							->where('mbs.id = :id')
							->setParameter('id', $mbsale_id)
							->orderBy('mbs.ref', 'ASC');
					},
					'choice_label' => 'id',
					'multiple' => false,
		...
		if (null == $this->mbsale) {
			...
			$company_id = $this->mbsale->getCompany()
				->getId();
			$builder->add('relation', EntityType::class,
				array(
					'label' => 'Sale.relation.label',
		...
		$builder->add('secondaryVats', CollectionType::class,
			array(
