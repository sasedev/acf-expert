...
class UpdateParentTForm extends AbstractType
{
		...
		if (null == $this->company) {
			$builder->add('parent', EntityType::class,
				array('label' => 'Docgroupperso.parent.label', 'class' => 'AcfDataBundle:Docgroupperso',
					'query_builder' => function (DocgrouppersoRepository $dgr) use ($selfUrl)
					{
			...
			$builder->add('parent', EntityType::class,
				array('label' => 'Docgroupperso.parent.label', 'class' => 'AcfDataBundle:Docgroupperso',
					'query_builder' => function (DocgrouppersoRepository $dgr) use ($selfUrl, $company_id)
					{
						$qb = $dgr->createQueryBuilder('d')
							->join('d.company', 'c')
							->where('c.id = :cid')
	...
	{
		return 'DocgrouppersoUpdateParentForm';
	}
